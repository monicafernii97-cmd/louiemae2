...
import { Menu, Search, ShoppingBag, X, Instagram, Facebook, Twitter, ChevronDown, ArrowRight, ChevronRight } from 'lucide-react';
      ...
      <div className="min-h-screen bg-cream font-sans selection:bg-bronze selection:text-white overflow-x-hidden">
        ...
        {activePage !== 'admin' && (
          <>
            {/* Mobile Menu Overlay */}
            <div 
              className={`fixed inset-0 z-50 flex flex-col bg-[#F9F7F2] transition-all duration-[800ms] ease-[cubic-bezier(0.22,1,0.36,1)] ${
            ...
            <header 
              className={`fixed top-0 left-0 right-0 z-40 transition-all duration-700 border-b ${
                scrolled ? 'bg-cream/90 backdrop-blur-md py-3 border-sand/40 shadow-sm' : 'bg-transparent py-6 border-transparent'
              }`}
            >
              <div className="w-full px-6 md:px-12 relative flex justify-between items-center min-h-[50px]">
                      ...
                      {extendedNavLinks.slice(3).map(link => (
                        <div key={link.label} className="relative group">
                          ...
                          >
                           ...
                           {link.children && (
                             <div className="absolute top-full left-1/2 -translate-x-1/2 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 ease-out transform origin-top translate-y-1 group-hover:translate-y-0">
                                <div className="bg-cream/95 backdrop-blur-xl border border-white/60 py-3 px-4 shadow-[0_10px_30px_-10px_rgba(74,59,50,0.15)] rounded-2xl min-w-[140px] flex flex-col gap-1 text-center relative overflow-hidden">
                                   <div className="absolute inset-0 bg-gradient-to-b from-white/40 to-transparent pointer-events-none"></div>
                                   {link.children.map(child => (
                                      <button
                                         key={child.label}
                                         onClick={(e) => { e.stopPropagation(); handleNavigation(child.href); }}
                                         className="relative z-10 text-[0.6rem] uppercase tracking-[0.2em] text-earth/70 hover:text-bronze transition-all duration-300 whitespace-nowrap py-1.5 hover:bg-white/50 rounded-lg"
                                      >
                                         {child.label}
                                      </button>
                                   ))}
                ...
                <div className="flex items-center gap-4 md:gap-6 ml-auto z-50 pl-4">
                  <button className="text-earth hover:text-bronze transition-colors p-1">
                    <Search className="w-4 h-4 md:w-5 md:h-5" />
                  </button>
                  <button onClick={() => handleNavigation('#shop')} className="text-earth hover:text-bronze transition-colors relative p-1">
                    <ShoppingBag className="w-4 h-4 md:w-5 md:h-5" />
                    <span className="absolute -top-1 -right-1 w-1.5 h-1.5 bg-bronze rounded-full"></span>
                  </button>
                </div>
              </div>
            </header>
