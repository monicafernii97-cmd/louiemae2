...
class App {
    ...
    onTouchUp(e: MouseEvent | TouchEvent) {
        ...
        const x = "touches" in e && e.changedTouches ? e.changedTouches[0].clientX : (e as MouseEvent).clientX;
        const distance = Math.abs(this.start - x);

        if (distance < 5 && this.onItemClick) {
            // It was a click, find which item was clicked
            const clickedMedia = this.findClickedMedia(x);
            if (clickedMedia !== null) {
                const originalIndex = clickedMedia % (this.mediasImages.length / 2);
    ...
    findClickedMedia(clientX: number): number | null {
        // Calculate which media is closest to the center/click position
        const containerRect = this.container.getBoundingClientRect();
        const centerX = containerRect.width / 2;
        const clickOffset = clientX - containerRect.left - centerX;

        // Find the media closest to clicked position
        let closestIndex = 0;
        let closestDistance = Infinity;

        this.medias.forEach((media, index) => {
            const mediaX = media.plane.position.x * (containerRect.width / this.viewport.width);
            const distance = Math.abs(mediaX - clickOffset);
    ...
    return (
        <div
            ...
            className={cn(
                ...
                "text-earth font-bold text-[30px]",
                fontClassName,
                className,
            )}
            {...props}
